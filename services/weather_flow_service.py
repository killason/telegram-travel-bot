...
def process_weather_by_location(user_id: int, user_name: str, lat: float, lon: float) -> tuple[str, str]:
    ...
    save_city_set(user_id, city, weather_short)
    ...
    advice = get_ai_advice(city, weather["condition"], weather['temperature'], user_name=user_name)

    return (weather_text, f"💡 Совет: {advice}")
